...
public class PageDTO {
   ...
   private boolean prev;         // 다음 페이지목록의 존재여부, 이전  페이지로의 이동버튼을 생성할지 여부 
   private boolean next;         // 이전 페이지목록의 존재여부, 다음 페이지로의 이동버튼을 생성할지 여부  
   ...
   public PageDTO(Criteria cri, int totalAmount) {
      ...
      //----------------------------------------------------------//
      int currPage = cri.getCurrPage();				//출력시킬 현재 페이지 번호 추 from Criteria
      int amount = cri.getAmount();					//한 페이지당 보여줄 레코드 건수 추출 from Criteria
      int pagesPerPage = cri.getPagesPerPage();		//각 페이지 아래 보여줄, 페이지번호목록ㄱ의 길이 추출, from
